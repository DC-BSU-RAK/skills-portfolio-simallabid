...
class StudentManagerApp:
    ...
    def play_click_sound(self, event):
        ...
        if self.click_sound:
            # We check that the widget clicked is not the output text area (self.output_area)
            # This prevents sound spam when a user clicks and drags to select text.
    ...
    def _find_student(self, search_term):
        ...
        # 1. Check for Exact Student Number Match (Takes priority)
        for student in self.students:
            if student.student_number.lower() == search_term:
                return [student] 
        
        # 2. If no exact number match, perform Alphabetic/Prefix search
        for student in self.students:
    ...
    def add_student_record(self):
        ...
        # 1. Get student number and check for duplicates
        while True:
        ...
        # 3. Get coursework marks (max 20 each)
